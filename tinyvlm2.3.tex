...
\begin{itemize}
\item We propose \textbf{Sparse Temporal Token Fusion (STTF)}, a mechanism for exploiting temporal redundancy via token reuse, reducing token count by up to 84\% on event-based inputs (DVS128 Gesture, N-Caltech101; Tables~\ref{tab:dvs128}, \ref{tab:ncaltech}). On single-frame COCO, STTF retains the routing and decoder benefits but the temporal-cache pathway is necessarily inactive; full RGB-video validation (MSR-VTT, EgoSchema) is in progress and reported in supplementary.
