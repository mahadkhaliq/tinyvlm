...
\textbf{Yes.} All quantitative claims (CIDEr, BLEU-4, GFLOPs, DVS128 accuracy) in the abstract and introduction correspond to entries in Tables~\ref{tab:main} and~\ref{tab:ondevice} and are supported by 3-seed experiments with paired $t$-tests.

\item \textbf{Limitations.}
\textit{Does the paper discuss the limitations of the work performed by the authors?}\\
\textbf{Yes.} A dedicated Limitations paragraph (Section~6) identifies the CIDEr gap of the CLIP+GPT-2 path, fixed 2$\times$2 patch granularity of STTF, and the absence of full real-world event-RGB synchronization.
...
\textbf{Yes.} All multi-seed results report mean~$\pm$~std. Comparisons against TokenLearner use paired Welch $t$-tests with Cohen's $d$ effect sizes and 95\% confidence intervals (Table~\ref{tab:main}).
...
\textbf{Yes.} Appendix~A (Broader Impact) addresses positive applications (assistive technology, healthcare, disaster response) and risks (surveillance, privacy, safety in autonomous systems, aggregate energy consumption).
...
\textbf{NA.} No novel attack vectors or tools with significant misuse potential are introduced. Broader societal risks are discussed in Appendix~A.
