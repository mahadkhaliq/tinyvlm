...
\begin{itemize}
\item We propose \textbf{Sparse Temporal Token Fusion (STTF)}, a mechanism for exploiting temporal redundancy via token reuse, reducing token count by up to 84\% (validated in Table 1 and Table 3).
...
\textbf{CLIP + GPT-2 backbone}: extends the CLIP variant by replacing the scratch-trained decoder with a pretrained GPT-2 small (117M parameters)~\cite{radford2019gpt2}. A two-layer MLP visual prefix adapter projects ANC output (384-$d$) to 10 prefix tokens in GPT-2's embedding space (768-$d$); GPT-2 is then fine-tuned end-to-end alongside the prefix adapter and ANC heads. This configuration demonstrates that STTF+ANC routing is compatible with a fully pretrained vision-language backbone — both the visual encoder (CLIP) and the language decoder (GPT-2) are initialised from pretrained weights. Unless otherwise noted, Tables~\ref{tab:sttf_tau}--\ref{tab:main} report CNN-backbone results; CLIP and CLIP+GPT-2 results are reported in Table~\ref{tab:main}.
...
CIDEr for STTF+ANC is averaged over three seeds (42, 43, 44); all other methods are single-seed.
...
\begin{table}[htbp]
\centering
\caption{Comparison on MS-COCO captioning. \emph{CNN}: 21M-parameter scratch-trained backbone; \emph{CLIP}: frozen ViT-B/32 + MLP projection heads, 15 epochs, single seed. STTF+ANC (CNN) results are mean~$\pm$~std over seeds \{42,~43,~44\}; 95\% CI computed via $t_{0.025,\,df=2}=4.303$. Superscripts indicate one-sample $t$-tests ($^{**}p<0.01$, $^{***}p<0.001$). FLOPs for STTF+ANC (CNN) are the hard-routing average at inference: $0.31{\times}2 + 0.34{\times}8 + 0.35{\times}20 \approx 10.3$~GFLOPs.}
\label{tab:main}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lccccc}
\toprule
...
\textbf{STTF + ANC (Ours, CLIP)} & $\sim$\textbf{17.6 G} & \textbf{52.47} & \textbf{89.72} & \textbf{27.77} & \textbf{TBD} \\
\textbf{STTF + ANC (Ours, CLIP+GPT-2)} & $\sim$\textbf{17.6 G} & \textbf{53.91} & \textbf{69.09} & \textbf{—} & \textbf{TBD} \\
\midrule
\multicolumn{3}{l}{\small Trade-off vs TokenLearner ($^\dagger$STTF+ANC $<$ baseline):} & \small $-$8.4 CIDEr & & \small $-$50\% FLOPs, $-$2.6 ms \\
\bottomrule
\end{tabular}%
}
\end{table}
