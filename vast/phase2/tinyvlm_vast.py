...
try:
    from clip_token_budget import TokenBudgetCLIPStem, RGBComplexityEstimator  # Phase-2
    _TOKEN_BUDGET = True
except Exception:
    _TOKEN_BUDGET = False
...
class Config:
    ...
    token_select: str = "stride"      # E21: "stride" (pre-registered) | "attn" (CLS-attention top-k)
    single_branch_keep_k: Optional[int] = None  # E20: bypass the router; run ONE branch at this budget
...
def greedy_decode_gpt2(
    ...
    if budgets is not None:
        # Phase-2: route from RGB, run only the selected branch's budgeted stem.
        router_logits = raw.complexity_estimator(rgb)
        branch_idx = F.softmax(router_logits, dim=-1).argmax(dim=-1)
        for i, (enc, proj, k) in enumerate(zip(raw.encoders, raw.projections, budgets)):
            mask = branch_idx == i
            if not mask.any():
                continue
            feat = enc(raw.clip_stem(rgb[mask], keep_k=k, select=raw.cfg.token_select))
...
class CLIPStemEncoder(nn.Module):
    ...
    # CORRECTED 2026-07-16: was 17.6e9, the canonical ViT-B/**16** figure, on a ViT-B/**32** model
    # (~4.1x too large). 4.29e9 = (12*N*d^2 + 2*N^2*d)*L at N=50, d=768, L=12, counting MACs as
    # FLOPs -- the convention under which B/16 is 17.6 G and B/32 is 4.4 G. The iso-cost conclusion
    # is unchanged either way: the per-branch heads below differ by ~0.13 M against a ~4.29 G stem.
    FLOPS: float = 4.29e9
...
class AdaptiveNeuralCompression(nn.Module):
    ...
    def __init__(self, cfg: Config) -> None:
        ...
        self._budgets = None   # Phase-2: set when clip_token_budget
        self._force_branch: Optional[int] = None  # E22: oracle eval pins every input to one branch

        if cfg.clip_backbone and cfg.clip_token_budget:
            # Phase-2: three branches process different visual-token budgets INSIDE the
            # frozen ViT, so per-branch STEM FLOPs genuinely differ. Router reads a cheap
            # RGB signal (not the CLS) so routing does not require the full stem and is
            # image-dependent (fixes the E8 zero-signal collapse).
            ...
            self.encoders = nn.ModuleList([
                nn.Sequential(nn.Linear(clip_dim, d), nn.GELU(), nn.Dropout(cfg.dropout))
                for d in head_dims
            ])
            self.projections = nn.ModuleList([
                nn.Linear(d, cfg.hidden_dim) for d in head_dims
            ])
            if cfg.single_branch_keep_k is not None:
                # E20: keep exactly the branch that owns this budget -- same stem, same head
                # dim, same decoder as that branch had inside the routed model -- and drop the
                # router entirely. This is the routed model minus routing, which is what makes
                # it a clean attribution of the ~5 CIDEr penalty.
                if cfg.single_branch_keep_k not in budgets:
                    raise ValueError(
                        f"--single_branch_keep_k {cfg.single_branch_keep_k} not in "
                        f"--token_budgets {budgets}; it must name one of the routed branches."
                    )
                bi = budgets.index(cfg.single_branch_keep_k)
                budgets = [budgets[bi]]
                head_dims = [head_dims[bi]]
                self._budgets = budgets
                self.encoders = nn.ModuleList([self.encoders[bi]])
                self.projections = nn.ModuleList([self.projections[bi]])
                self.complexity_estimator = None   # no router at all
                logger.info(
                    "E20 single-branch: keep_k=%d, head_dim=%d, router BYPASSED "
                    "(no balance/entropy loss applies)",
                    budgets[0], head_dims[0],
                )
            else:
                self.complexity_estimator = RGBComplexityEstimator(n_branches=len(head_dims))
            self._head_flops = [
                self.clip_stem.stem_flops(k, self.cfg.token_select) + 2.0 * d * clip_dim
    ...
    def forward(
        ...
        if self._force_branch is not None and use_soft_routing:
            # Fail loudly rather than silently ignoring the override. Soft routing mixes all
            # branches, so a caller who sets _force_branch without model.eval() would get a
            # plausible-looking mixture and never know the pin did nothing. That is precisely
            # the class of silent wrong number this project has been burned by.
            raise RuntimeError(
                "_force_branch is set but the model is in soft-routing mode "
                f"(training={self.training}, eval_routing_mode={self.cfg.eval_routing_mode!r}). "
                "The override only applies on the hard-argmax path -- call model.eval() and use "
                "eval_routing_mode='hard'."
            )

        if self._budgets is not None and self.cfg.single_branch_keep_k is not None:
            # E20: one branch, no router. Emit degenerate router tensors so the loss/logging
            # plumbing is unchanged: a single-expert softmax has zero entropy and a
            # load-balance value of exactly 1.0 (its minimum), so both terms are inert here.
            k = self._budgets[0]
            feat = self.encoders[0](self.clip_stem(rgb, keep_k=k, select=self.cfg.token_select))
            encoded = self.projections[0](feat)
            comp_cost = comp_cost + self._encoder_flops(0)
            router_logits = torch.zeros(B, 1, device=rgb.device)
            weights = torch.ones(B, 1, device=rgb.device)

        elif self._budgets is not None:
            # Phase-2 token-budget path: route from a cheap RGB signal, then run a
            # per-branch budgeted stem (soft = all branches at train; hard = only the
            # argmax branch at inference, realising that branch's stem FLOPs).
            router_logits = self.complexity_estimator(rgb)
            weights = self.router(router_logits)
            if use_soft_routing:
                for i, (enc, proj, k) in enumerate(zip(self.encoders, self.projections, self._budgets)):
                    feat = enc(self.clip_stem(rgb, keep_k=k, select=self.cfg.token_select))
                    w = weights[:, i : i + 1]
                    encoded = encoded + w * proj(feat)
                    comp_cost = comp_cost + w.squeeze(-1) * self._encoder_flops(i)
            else:
                if self._force_branch is not None:
                    # E22 oracle: ignore the learned router and send everything to one branch,
                    # so we can score each branch over the whole split and take the per-image
                    # best. This is an upper bound on ANY routing policy at these budgets.
                    branch_idx = torch.full(
                        (B,), self._force_branch, dtype=torch.long, device=rgb.device
                    )
                else:
                    branch_idx = weights.argmax(dim=-1)
                for i, (enc, proj, k) in enumerate(zip(self.encoders, self.projections, self._budgets)):
                    ...
                    if not mask.any():
                        continue
                    feat = enc(self.clip_stem(rgb[mask], keep_k=k, select=self.cfg.token_select))
                    encoded[mask] = proj(feat)
...
@torch.no_grad()
def audit_token_budget_routing(trainer: Any) -> Dict[str, Any]:
    """Phase-2 A2/A3/A4 instrumentation: over the eval loader, tally which branch the
    RGB router selects at hard-argmax, the realised (utilisation-weighted) stem FLOPs,
    and the complexity-vs-branch Spearman correlation."""
    raw = trainer.model.module if hasattr(trainer.model, "module") else trainer.model
    budgets = getattr(raw, "_budgets", None)
    if budgets is None or raw.clip_stem is None:
        return {}
    if getattr(raw.cfg, "single_branch_keep_k", None) is not None:
        # E20: the router is bypassed by construction, so A3/A4 are not applicable rather
        # than failed. Report the fixed budget and its realised cost; do not fabricate a
        # routing distribution for a model that has no router.
        k = budgets[0]
        return {
            "single_branch": True,
            "keep_k": k,
            "token_select": raw.cfg.token_select,
            "inference_branch_fractions": [1.0],
            "realised_encoder_gflops": raw.clip_stem.stem_flops(k, raw.cfg.token_select) / 1e9,
            "max_branch_fraction": 1.0,
            "complexity_branch_spearman": None,
            "note": "router bypassed (--single_branch_keep_k); A3/A4 not applicable",
        }
    raw.eval()
    K = len(budgets)
    ...
    margins: List[float] = []
    branches: List[int] = []
    for rgb, events, tokens in trainer.val_loader:
        rgb = rgb.to(trainer.device, non_blocking=True)
        probs = torch.softmax(raw.complexity_estimator(rgb), dim=-1)
        bidx = probs.argmax(dim=-1)
        counts += torch.bincount(bidx.cpu(), minlength=K).float()
        top2 = probs.topk(2, dim=-1).values
        margins.extend((top2[:, 0] - top2[:, 1]).cpu().tolist())
        branches.extend(bidx.cpu().tolist())
    total = float(counts.sum().item()) or 1.0
    frac = (counts / total).tolist()
    realised = sum(f * raw.clip_stem.stem_flops(k, raw.cfg.token_select) for f, k in zip(frac, budgets)) / 1e9
    rho = None
    try:
        from scipy.stats import spearmanr
        if len(set(branches)) > 1:
            rho = float(spearmanr(margins, branches).correlation)
    except Exception:
        rho = None
    return {
        "inference_branch_fraction": [round(f, 4) for f in frac],
        "realised_encoder_gflops": round(realised, 3),
        "complexity_branch_spearman": rho,
        "n_eval_images": int(total),
        "token_budgets": budgets,
        "per_branch_stem_gflops": [round(raw.clip_stem.stem_flops(k) / 1e9, 2) for k in budgets],
    }
...
def run_eval_only(cfg: Config) -> Dict[str, Any]:
    ...
    routing_audit = audit_token_budget_routing(trainer) if cfg.clip_token_budget else {}
    summary = {
        "run_dir": str(run_dir),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--single_branch_keep_k", type=int, default=None,
                   help="E20: bypass the router and run ONE branch at this token budget "
                        "(must be one of --token_budgets). Isolates token-dropping cost from the "
                        "cost of the routed 3-branch configuration. Use --lambda_balance 0 "
                        "--lambda_entropy 0; both terms are inert with a single expert.")
    p.add_argument("--token_select", choices=["stride", "attn"], default="stride",
                   help="E21: token-selection rule for --clip_token_budget branches. "
                        "'stride' = pre-registered even stride (ONNX-exportable). "
                        "'attn' = EViT-style CLS-attention top-k (data-dependent gather, "
                        "NOT ONNX-exportable, and ~3-8%% more FLOPs -- stem_flops() accounts for it).")
...
def config_from_args(args: argparse.Namespace) -> Config:
    return Config(
        ...
        single_branch_keep_k=args.single_branch_keep_k,
